...
function init() {
  ...
  try {
    ...
    const hasIsAdmin = cols.some((c) => c && c.name === "isAdmin")
    if (!hasIsAdmin) {
      db.prepare(
...
function seedIfEmpty() {
  ...
  try {
    module.exports.createCourse({
      ...
      description:
        "Пределы, непрерывность, производные, интегралы. Тексты лекций согласованы по темам с учебником А. А. Гусака «Высшая математика» (т. 1); в курсовой укажите издание и параграфы.",
...
Предел описывает, к какому значению «стремится» $f(x)$, когда $x$ сколь угодно приближается к $x_0$, не обязательно совпадая с $x_0$. Это понятие связывает дискретные приближения с точным объектом анализа и дальше используется при определении непрерывности, производной и интеграла.

Говорят, что $L$ — **предел** $f(x)$ при $x \\to x_0$, и пишут

$$
\\lim_{x \\to x_0} f(x) = L,
$$

если для любого $\\varepsilon > 0$ найдётся $\\delta > 0$ такое, что из условия $0 < |x - x_0| < \\delta$ следует $|f(x) - L| < \\varepsilon$ (при условии, что $f$ определена в проколотой окрестности $x_0$).

**Односторонние пределы** $f(x_0-0)$ и $f(x_0+0)$ изучают поведение только слева или справа; равенство всех трёх величин ($L$, левый и правый пределы) важно при разборе разрывов.

Свойства (при существовании пределов): предел единствен; линейная комбинация, произведение и частное (если знаменатель не стремится к нулю) вычисляются по покомпонентным правилам.

> **Литература для теории (укажите в работе издание и параграфы):** Гусак А. А. *Высшая математика*, т. 1 — разделы о пределе функции и его свойствах.
`,
      ord: 1,
    },
...
Непрерывность формализует идею «без скачка»: малое изменение аргумента влечёт малое изменение значения функции. Это прямое следствие согласованности $\\lim_{x \\to x_0} f(x)$ с $f(x_0)$.

**Непрерывность в точке** $x_0$ означает:
1. $f(x_0)$ существует;
2. существует $\\lim_{x \\to x_0} f(x)$;
3. $\\displaystyle \\lim_{x \\to x_0} f(x) = f(x_0)$.

Если свойство выполнено в каждой точке интервала (отрезка), говорят о непрерывности на этом множестве. Сумма, произведение, композиция и частное (где знаменатель не обнуляется) непрерывных функций снова непрерывны на соответствующих областях.

На отрезке непрерывная функция **ограничена** и достигает наибольшего и наименьшего значений; между любыми двумя значениями на отрезке она принимает любое промежуточное (теорема о промежуточном значении).

> **Литература:** Гусак А. А. *Высшая математика*, т. 1 — непрерывность, точки разрыва, свойства непрерывных на отрезке функций.
`,
      ord: 2,
    },
...
Производная измеряет (локально) скорость изменения $f$ относительно $x$: это предел приращения функции к приращению аргумента.
...
**Механический смысл:** если $s(t)$ — координата по времени, то $s'(t)$ интерпретируют как мгновенную скорость.

Дифференцируемость в точке влечёт непрерывность там же; обратное неверно (пример: $|x|$ в нуле). Удобная запись: $df = f'(x)\\,dx$ при работе с дифференциалами.

> **Литература:** Гусак А. А. *Высшая математика*, т. 1 — производная, касательная, дифференциал.
`,
      ord: 3,
    },
...
Ниже — стандартные формулы для элементарных функций (в областях определения):

- $(C)' = 0$, $\\quad (x^n)' = n x^{n-1}$
...
**Правила:** линейность $(u \\pm v)' = u' \\pm v'$; произведение $(uv)' = u'v + uv'$; частное $\\left(\\dfrac{u}{v}\\right)' = \\dfrac{u'v - uv'}{v^2}$; **цепное** $(f(g(x)))' = f'(g(x))\\,g'(x)$.

Они позволяют дифференцировать рациональные, показательные, логарифмические и тригонометрические выражения и составные функции.

> **Литература:** Гусак А. А. *Высшая математика*, т. 1 — таблица производных, правила дифференцирования.
`,
      ord: 4,
    },
...
Операция, обратная дифференцированию: по заданной $f$ ищут такую $F$, что $F'(x) = f(x)$. Всякая первообразная отличается на постоянную.

$$
\\int f(x)\\, dx = F(x) + C, \\qquad F'(x) = f(x).
$$

**Методы:** линейность интеграла; **подстановка** $\\int f(g(x))g'(x)\\,dx = \\int f(u)\\,du$; **по частям** $\\int u\\,dv = uv - \\int v\\,du$. Имеет смысл опираться на таблицу базовых интегралов (степени, рациональные дроби, показательная, тригонометрия).

Неопределённый интеграл нужен при восстановлении функции по скорости, в физике и при решении простейших дифференциальных уравнений.

> **Литература:** Гусак А. А. *Высшая математика*, т. 1 — неопределённый интеграл, методы интегрирования.
`,
      ord: 5,
    },
...
На отрезке $[a,b]$ интеграл $\\displaystyle \\int_a^b f(x)\\,dx$ задаёт «накопленную сумму» с бесконечной точностью (предел интегральных сумм Римана для интегрируемых функций). Если $f \\ge 0$, величина совпадает с площадью под графиком.

**Формула Ньютона–Лейбница:** если $F$ — первообразная $f$ на $[a,b]$, то

$$
\\int_a^b f(x)\\, dx = F(b) - F(a) = F(x)\\Big|_a^b.
$$

Свойства: линейность, аддитивность по отрезку, оценка модуля через $\\sup |f|$. Приложения: площади, работа силы, некоторые объёмы и средние значения.

> **Литература:** Гусак А. А. *Высшая математика*, т. 1 — определённый интеграл, формула Ньютона–Лейбница, геометрические приложения.
`,
      ord: 6,
    },
  ]
...
function seedAdmin() {
  ...
  try {
    const existing = module.exports.getUserByEmail(email)
    if (!existing) {
      module.exports.createUser({
        id: `admin-${uuidv4()}`,
        email,
